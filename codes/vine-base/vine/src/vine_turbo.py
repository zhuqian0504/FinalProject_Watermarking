...
def initialize_unet_no_lora(path=base):
...
def initialize_vae_no_lora(path=base):
...
class VINE_Turbo(torch.nn.Module, PyTorchModelHubMixin):
    def __init__(self, ckpt_path=None, device='cuda'):
        ...
        self.emb_proj = nn.Linear(768, 2048).to(device)
    ...
    def forward(self, x, timesteps=None, secret=None):
        ...
        # 2. 创建符合要求的 time_ids（适配 sample_size=128，对应原始图像 512x512）
        # 格式：[original_height, original_width, target_height, target_width, crop_top, crop_left]
        # 由于模型 sample_size=128，是原始图像的 1/4（512/4=128），因此：
        original_height = 512
        original_width = 512
        target_height = 128
        target_width = 128
        ...
        time_ids = torch.tensor(
            [[original_height, original_width, target_height, target_width, crop_top, crop_left]] * batch_size,
            dtype=torch.long,  # 必须是 long 类型
            device=device
        )

        # 3. 创建空的 text_embeds（模型要求传入，维度需匹配 cross_attention_dim=2048）
        # 注意：text_embeds 形状为 (batch_size, 1, cross_attention_dim)
        text_embeds = torch.zeros((batch_size, 1280), device=device, dtype=x_enc.dtype)
        projected_fixed_emb = self.emb_proj(self.fixed_a2b_emb_base)


        model_pred = self.unet(x_enc, timesteps, encoder_hidden_states=projected_fixed_emb,added_cond_kwargs={"text_embeds":text_embeds,"time_ids":time_ids}).sample.to(x.dtype)
