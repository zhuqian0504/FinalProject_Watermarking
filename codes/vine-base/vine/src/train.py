...
from vine_turbo import VINE_Turbo, VAE_encode, VAE_decode
from training_src.training_utils import parse_args
from training_src.stega_utils import StegaData, get_secret_acc, count_parameters
import training_src.extra_utils as extra_utils
from kornia import color
from vine.src.stega_encoder_decoder import CustomConvNeXt
from training_src.transformations import TransformNet
from training_src.wm_modules import Discriminator
import bchlib
import os
...
base="/root/autodl-tmp/local-base"
from accelerate.utils import DeepSpeedPlugin


class TrainModel(torch.nn.Module):
    """
    包装所有需要由 Accelerator/DeepSpeed 管理的子模块，避免在 DeepSpeed 模式下
    出现 “同一个 Accelerator 实例管理多个模型” 的断言错误。
    """

    def __init__(self, watermark_encoder, net_disc_a, decoder, transform_net, net_lpips):
        super().__init__()
...
def main(args):
    dataset_train = StegaData(args.dataset_folder, args.secret_size, size=(IMAGE_SIZE, IMAGE_SIZE)).get_datasets()
    ...
    if accelerator.state.deepspeed_plugin is not None:
        # 优先从命令行参数读取batch size，若无则设默认值（根据你的GPU显存调整）
        batch_size = getattr(args, 'train_batch_size', 4)  # 假设args有batch_size参数，无则默认4
        accelerator.state.deepspeed_plugin.deepspeed_config['train_micro_batch_size_per_gpu'] = batch_size
        # 可选：同步gradient_accumulation_steps
        # accelerator.state.deepspeed_plugin.deepspeed_config['gradient_accumulation_steps'] = args.gradient_accumul
    ...
    watermark_encoder = VINE_Turbo(device=accelerator.device)
    decoder = CustomConvNeXt(secret_size=args.secret_size)
    ...
    tokenizer = AutoTokenizer.from_pretrained(base, subfolder="tokenizer", revision=args.revision, use_fast=False,)
    ...
    # 使用单一的复合模型对象交由 Accelerator（以及 DeepSpeedPlugin）管理，
    # 避免 DeepSpeed 下对多个独立模型调用 `prepare` 引发的断言错误。
    ...
    train_model, optimizer_gen, optimizer_sec, optimizer_disc, train_dataloader, lr_scheduler_sec, lr_scheduler_gen, lr_scheduler_disc = accelerator.prepare(
        train_model, optimizer_gen, optimizer_sec, optimizer_disc, train_dataloader, lr_scheduler_sec, lr_scheduler_gen,
        lr_scheduler_disc
    )

    # 重新取回各子模块引用，后续训练逻辑保持不变
    watermark_encoder = train_model.watermark_encoder
    net_disc_a = train_model.net_disc_a
    decoder = train_model.decoder
    transform_net = train_model.transform_net
    net_lpips = train_model.net_lpips

    # 非 DeepSpeed 模式下，仍然手动设置 dtype；DeepSpeed 下由引擎管理。
    if not getattr(args, "use_deepspeed", False):
    ...
    val_img = glob(os.path.join(args.val_folder, "*.png"))
    fixed_img_a = None
    set_seed(args.seed)
