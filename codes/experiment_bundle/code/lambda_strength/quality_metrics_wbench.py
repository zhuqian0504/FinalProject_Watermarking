        ...
        if c == 'LOCAL_EDITING_5K':
            ...
            for cs in sub_category:
                ...
                if not os.path.isdir(image_folder):
                    print(f"[INFO] Skip {image_folder}, folder not found.")
                    continue

                wbench_folder = os.path.join(args.wbench_path, c, cs, 'image')
                if not os.path.isdir(wbench_folder):
                    print(f"[INFO] Skip {wbench_folder}, folder not found.")
                    continue
            ...
            wbench_folder = os.path.join(args.wbench_path, c, 'image')
            if not os.path.isdir(wbench_folder):
                print(f"[INFO] Skip {wbench_folder}, folder not found.")
                continue
