...
#cys改的，低频削弱攻击
POSSIBLE_D = [1, 3, 5, 10, 20, 50, 100, 200]
def freq_lowpass_attack(x, D0):
    orig_dtype = x.dtype
    x32 = x.to(torch.float32)
    X = torch.fft.fft2(x32)        # [B, C, H, W], complex
    X = torch.fft.fftshift(X)
    B, C, H, W = X.shape
    yy, xx = torch.meshgrid(
        torch.arange(H, device=x32.device) - H // 2,
        torch.arange(W, device=x32.device) - W // 2,
        indexing="ij"
    )
    D2 = xx**2 + yy**2
    D0_sq = float(D0) ** 2
    H_hp = 1.0 - torch.exp(- D2 / (2.0 * D0_sq))    # [H, W]
    H_hp = H_hp[None, None, :, :]                   # [1,1,H,W]
    X_att = X * H_hp
    X_att = torch.fft.ifftshift(X_att)
    x_att = torch.fft.ifft2(X_att).real             # 取实部
    return x_att.to(orig_dtype)
def main(args):
    ...
    for epoch in range(first_epoch, args.max_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*l_acc):
                ...
                #cys改的带开关的训练阶段随机D低频削弱攻击
                encoded_image = watermark_encoder(img_a, timesteps, secret)
                encoded_warped = 0.5 * (encoded_image + 1)
                if True:
                    D0 = float(np.random.choice(POSSIBLE_D))
                    encoded_image_attacked = freq_lowpass_attack(encoded_image, D0)
                else:
                    encoded_image_attacked = encoded_image
                transformed_image = transform_net(encoded_image_attacked, img_a, global_step, args)  # [-1, 1]
                transformed_image = transformed_image * 0.5 + 0.5  # [-1, 1] -> [0, 1]

                decoded_secret = decoder(transformed_image.to(dtype=weight_dtype))
                    ...
                    if global_step % args.validation_steps == 0:
                        ...
                        with torch.no_grad():
                            for i in range(len(val_img)):
                                ...
                                #cys改的验证阶段加入低频削弱攻击，示例固定D0=50
                                encoded_image = watermark_encoder(input_image, timesteps_val, secret_val)
                                if True:
                                    D0_val = 50.0
                                    encoded_image_attacked = freq_lowpass_attack(encoded_image, D0_val)
                                else:
                                    encoded_image_attacked = encoded_image
                                transformed_image_val = transform_net(encoded_image_attacked, input_image, global_step,args)
