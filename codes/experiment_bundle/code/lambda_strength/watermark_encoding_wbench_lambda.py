...
if __name__ == "__main__":
    ...
    parser.add_argument('--output_dir', type=str, default='./vine_encoded_wbench',
                        help='the directory to save the output')
    parser.add_argument('--pretrained_model_name', type=str, default='Shilin-LU/VINE-R-Enc',
                        help='pretrained_model_name')
    parser.add_argument('--message', type=str, default='Hello World!', help='the secret message to be encoded')
    parser.add_argument('--load_text', type=str, default='y',
                        help='the flag to decide to use inputed text or random bits')
    parser.add_argument('--lambda_strength', type=float, default=1.0,
                        help='scaling factor for watermark residual, larger = stronger watermark')
    args = parser.parse_args()

    print(f"[INFO] lambda_strength = {args.lambda_strength}")
    ...
    total_time = 0
    with torch.no_grad():
        category = ['DET_INVERSION_1K', 'INSTRUCT_1K', 'STO_REGENERATION_1K',
                    'LOCAL_EDITING_5K', 'SVD_1K', 'DISTORTION_1K']
        for c in category:
            if c == 'LOCAL_EDITING_5K':
                ...
                for cs in sub_category:
                    ...
                    if not os.path.isdir(source):
                        print(f"[INFO] Skip {source}, folder not found.")
                        continue
                    files = os.listdir(source)
                    for i in tqdm(files, desc="Processing images"):
                        input_path = os.path.join(source, i)
                        save_loc = os.path.join(args.output_dir, resolution, c, cs,
                                                os.path.split(i)[-1][:-4] + '_wm.png')
                        if not os.path.exists(save_loc):
                            ...
                            # 用 lambda_strength 调整水印强度
                            scaled_residual_512 = args.lambda_strength * residual_512

                            # ⭐ 关键：用 scaled_residual_512，而不是原始 residual_512
                            encoded_image = scaled_residual_512 + input_image  # 512x512 or original size
                ...
                for i in tqdm(files, desc="Processing images"):
                    input_path = os.path.join(source, i)
                    save_loc = os.path.join(args.output_dir, resolution, c,
                                            os.path.split(i)[-1][:-4] + '_wm.png')
                    if not os.path.exists(save_loc):
                        input_image = Image.open(input_path).convert('RGB')
                        ...
                        # ⭐ 关键：用 scaled_residual_512，而不是 residual_512
                        encoded_image = scaled_residual_512 + input_image  # 512x512 or original size
                        encoded_image = encoded_image * 0.5 + 0.5
                        encoded_image = torch.clamp(encoded_image, min=0.0, max=1.0)
